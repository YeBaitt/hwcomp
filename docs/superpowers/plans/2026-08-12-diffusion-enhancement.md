...
```python
# tests/conftest.py 追加（注意：必须注册 --gpu 选项，否则 addopts="--gpu" 会在
# 每次 pytest 启动时因"未识别参数"直接失败）
import pytest
...
- [ ] **Step 3: 实现 stage2（首选进程内 Python API + subprocess 回退）**

> **API 事实（Task 7 已确认，见 docs/api-notes.md，本步骤的权威依据）：** DiffBIR v2.1 入口是 `inference.py`（不存在 `scripts/restoration.py`，也没有 `--tiled`/`--aligned` 统一开关）。同分辨率恢复要分别指定四个组件的 tiled 开关（`--cleaner_*`/`--vae_encoder_*`/`--vae_decoder_*`/`--cldm_*`），`--input`/`--output` 必须是目录。`pipeline.run()`（diffbir/pipeline.py:236）的内部流程是 `apply_cleaner→apply_cldm→wavelet_reconstruction`，且断言 control 图各维 ≥512。进程内替换 stage-1 的最简做法：把 `pipeline.apply_cleaner` 换成恒等函数（api-notes.md 策略 B 的进程内版），再用我们 NAFNet 输出作为 `run()` 的输入，run() 内部的小波重建/色彩转换全部复用。
...
def _build_pipeline():
    ...
    复用 BSRInferenceLoop（diffbir/inference/bsr_loop.py:26）：构造 args
    （对照 inference.py parse_args 的默认值，覆盖 version="v2.1"、task="sr"、
    upscale=1、device、captioner="none"），loop.pipeline 即含 run/apply_cldm
    的 SR pipeline。
    """
...
def _make_args():
    """按 inference.py parse_args 默认构造 v2.1 SR 配置（读源码字段逐个对齐）。"""
    import argparse
    ...
    args.device = "cuda"
    args.captioner = "none"
    # 其余字段（seed、tile 相关等）对齐 inference.py parse_args 的默认值
    return args
...
def _refine_inproc(control_image: np.ndarray, steps: int, guidance: float,
    ...
    if _pipeline is None:
        ...
        try:
            ...
            _pipeline.apply_cleaner = lambda lq, *a, **k: lq  # 不再做内部 stage-1
        finally:
            os.chdir(cwd)
    ctrl = _to_uint8(control_image)
    sample = _pipeline.run(
        ctrl, steps=steps, strength=1.0,
...
> **降级链（engine 层）：** 若进程内路径在真实 4K 下不稳定（如权重加载失败），Task 10 引擎可整体走 subprocess CLI 路径——把我们的 NAFNet 输出作为 `--input` 传入（此时 DiffBIR 内部 stage-1 会再处理一次，结果仍可用，仅不如直接控制精细）。
